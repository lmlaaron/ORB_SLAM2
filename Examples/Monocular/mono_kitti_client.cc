...
class OrbSLAMClient {
 public:
  ...
  void NewSLAM(const string &strVocFile, const string &strSettingsFile, const eSensor sensor, const bool bUseViewer) {

	NewSLAMRequest request;
	request.set_strvocfile(strVocFile);
	request.set_strsettingfile(strSettingFile);
	request.set_esensor(sensor);
  ...
  cv::Mat TrackMonocular(const cv::Mat &im, const double &timestamp) {
	...
	// deserialize cv::mat
	cv::Mat ret(reply->im_width(), reply->im_height(), reply->im_type(), (reply->im()).c_str() );	
	return ret;
  }
  void Shutdown() {
 ...
 private:
  std::unique_prt<OrbSLAM::Stub> stub_;
}
